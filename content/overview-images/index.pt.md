...
description = "As imagens podem ser incorporadas diretamente com Markdown `![Ferris](ferris.svg)`, mas o uso de um componente evita CLS ao definir explicitamente a largura e a altura."
draft = false
title = "Componentes de Imagem"

[extra]
keywords = "Imagem, Markdown, Componentes, Troca"
series = "Recursos"
...
+++
This post covers the **imgswap and img components**. Imagens can also be embedded directly using Markdown `![Ferris](ferris.svg)`, but using a component lets you explicitly set the width and height. This helps prevent cumulative layout shift (CLS), improving page stability and potentially improving Lighthouse performance scores.

<!-- more -->

## Componente img

- src é o caminho e o nome do arquivo da imagem. (obrigatório)
...
- link define um hiperlink para a imagem.
...
*O Google Lighthouse recomenda definir os atributos alt, w e h. Se alt for omitido, o nome do arquivo será usado. Se w ou h forem omitidos, get_image_metadata() preencherá esses valores.*
...
O componente imgrow coloca vários componentes img ou imgswap lado a lado. Em telas pequenas, as imagens são empilhadas verticalmente de forma automática.

Uma `class` opcional pode ser adicionada ao contêiner da linha.
...
As imagens também podem ser empilhadas verticalmente colocando vários componentes img ou imgswap um após o outro.

{{<img src="ferris-happy.svg" alt="Ferris is Happy" />}}
{{<img src="ferris-gesture.svg" alt="Ferris Gesturing" />}}

## Imagem SVG diretamente no código
...
Por padrão, `main_src` é a imagem exibida e `swap_src` aparece ao passar o mouse.

Ao passar o mouse sobre a imagem, a imagem alternativa será exibida para comparação.

- main_src is the path and filename for the default image. (obrigatório)
- swap_src is the path and filename for the swap image. (obrigatório)
- class define uma classe para a imagem.
- alt define o texto alternativo da imagem.
- caption define o texto da legenda abaixo da imagem.
- link define um hiperlink para a imagem.
